...
if __name__ == "__main__":
    ...
    curr_n     = 8      # current number of monitoring paths
    ...
    # go through all paths and create a list of candidate paths (paths that are not our starting 4 paths)
    candidate_paths = []
    (timestep, path, folder) = sorted_files[0]
    data = get_lightpaths(path, top2_full)
    for item in data:
        path_list = item['path']    # list of nodes in monitoring path
        if (path_list in candidate_paths) or (len(path_list) < 2):  # filter out candidate paths that are either too short or duplicates
            continue
        if not (path_list in monitoring_paths):
            candidate_paths.append(path_list)

    # Choose how many augmented training sets to generate
    num_samples = 5
    extra_combos = random.sample(                           # random combinations of the candidate paths
        list(itertools.combinations(candidate_paths, 4)),
        num_samples
    )
    training_sets = [monitoring_paths + list(combo) for combo in extra_combos]

    for training_paths in training_sets:
