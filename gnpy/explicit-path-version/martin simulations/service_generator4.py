...
class GNPy_Simulator:
    ...
    def simulate(self, topology_file, eqpt_file):
        ...
        for r in range(self._rounds):
            ...
            fixed_path_counter  = 0
            num_fixed_paths     = 4 # number of fixed paths to take from the above list before randomly genereating the rest

            for n in range(self._num_req - len(active_cnxns)):

                if fixed_path_counter < num_fixed_paths:    # take from fixed paths (n times)
                    ...
                    fixed_path_counter += 1
                    print(fixed_path_counter)

                else:   # randomly geneate the rest of the paths
