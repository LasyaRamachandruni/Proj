...
class Optical_Monitoring:
    ...
    def __init__(self, broker: nx.Graph, trail_selection_policy: str = "random", cooperation: bool = False) -> None:
        # graphs
        ...
        # cooperation level
        self.coop = cooperation     # true = some cooperation, false = no cooperation
    ...
    def trail_select_least(self, candidate_trails: list) -> list:
        ...
        for i in range(len(candidate_trails)):  # iterate through all candidate trails
            ...
            candidate_trail_monitoring_scores[i] /= (len(ct)-1)    # normalize based on number of edges (don't prioritize short length paths)
    ...
    def select_link_failure_test(self, input_edge_ls: list) -> int:
        """
        Causes soft failure in one specific edge.
        Counts the number of returned
        links returned by localize() funciton in each iteration. This means,
        the lower the number, the better graph monitoring setup.
        ...
        Given m = number of edges in broker graph, G
        Upper bound = m   (return all links per edge; worst case)
        Lower bound = 1     (return 1 link per edge; optimal)
        """
        target_edge_ls = []
        edge_set = self.broker.edges
        for target_edge in input_edge_ls:
            (src, dst) = target_edge
            if target_edge not in edge_set:
                if (dst, src) not in edge_set:
                    raise Exception("Fiber not in broker graph!")
                else:
                    target_edge_ls.append((dst, src))
            else:
                target_edge_ls.append(target_edge)
...
if __name__ == "__main__":
    ...
    print("test")
    DTest = create_domain_star("1", 4)
    #nx.draw(DTest, with_labels = True)
    #plt.show()
    ...
    #abstracted_D1 = abstract_domain_star(D1, ["d1_v1"], "1")
    #abstracted_D1 = abstract_domain_star(D1, ["d1_v1", "d1_v2", "d1_v3", "d1_v4", "d1_v5"], "1")
    #nx.draw(abstracted_D1, with_labels = True)
    #plt.show()

    m1 = Optical_Monitoring(D1, "least")       # least covered path/cycle selection
    #m1.display_broker()

    #m1.store_domain_info(D1, ["d1_v1", "d1_v2"], "1")
    #m1.store_domain_info(D1, ["d1_v1", "d1_v2", "d1_v3", "d1_v4", "d1_v5"], "1")

    m1.add_monitoring_nodes(["d1_v1", "d1_v3"])

    #print("cycle")
    m1.trail_testing("d1_v1", "d1_v1")  # cycle

    #print("path")
    m1.trail_testing("d1_v1", "d1_v3")  # path

    m1.add_monitoring_trail(['d1_v1', 'd1_v2', 'd1_v5', 'd1_v4', 'd1_v3'])

    m1.remove_monitoring_trail(['d1_v1', 'd1_v2', 'd1_v5', 'd1_v4', 'd1_v3'])
